<%@ Page Language="vb" AutoEventWireup="false" Codebehind="resDocumentos.aspx.vb" Inherits="SisCajas.resDocumentos" %>
	   ...
	   //PROY-31949 - I
		...
		//RSExecute(serverURL,"ObtenerTipoPos",objCombo.value,document.getElementById("HidPtoVenta").value,CallBack_ObtenerTipoPos,ObtenerTipoPosError,"X"); //PROY-31949 
			...
			function f_EnvioPOS(intFila,TagOpcion)
			{
				...
				var varNameLinkDelPOS;
		                //var varNameDelFila;
	                        var varNameTipoDoc; var varNameMonto;
		                //var vaNameLink;
			...
			function CallService(tipoPOS,NameDoc,objEntityPOS){
				...
				var varIdTransaccion = document.getElementById("txtIdentificador").value + '_' + formatDate(VarToday);
		...
		function obtieneCalculoTotalFormasPago()
		{
			var suma = 0;
			for (i=1; i<=3; i++)	{
				val = getValue('txtMonto' + i);						
				if (val != '') 
					suma = suma + val*1;				
			} 
			//---
		...
		function f_Grabar(){
		    var blnErr = false;
			if (frmPrincipal.hdnEjecutado.value != "R")
			{
			if (document.getElementById("txtMonto1") != null)
			{
					if (f_ValidarFormasPago()) {
					CalculoVuelto();
					...
					if 	(!blnErr)
					{
							...
							if(confirm("¿Está seguro de registrar el pago?")){
								...
								frmPrincipal.hdnEjecutado.value = "R";
							}else
								event.returnValue = false;	
							//INC000001566216-FIN
					}
...
//e75810 no se utiliza
		function f_NumFilas(){
			var cont;
			var chk = f_ValidarFormasPago();	
			cont=0;
			if(chk == true){
				for(i=1; i<4; i++){
		...
		{
			var hldComando = document.getElementById("divComandos");
			var hldMensaje = document.getElementById("divMensaje");
			hldComando.style.visibility = "hidden";
			hldMensaje.style.visibility = "visible";
		}
		...
		function ShowCommands()
		{
			var hldComando = document.getElementById("divComandos");
			var hldMensaje = document.getElementById("divMensaje");
			hldComando.style.visibility = "visible";
			hldMensaje.style.visibility = "hidden";
		}
		...
		{
			var objRefs = document.getElementsByName("strReferencia");
			var objMonts = document.getElementsByName("strMontoPagar");
			var dblMontoTotal = 0;

			for (i=0; i<objRefs.length; i++){
				if (objRefs.item(i).value.length>0){
					if (!ValReferencia(objRefs.item(i).value, "Nro Tarjeta/Nro Cheque")){
						ShowCommands();
						return false;
					}
				}

				if (objMonts.item(i).value.length>0){
					if (!ValNumPunto(objMonts.item(i).value, "Monto a Pagar")){
						ShowCommands();
						return false;
					}
					if (isNaN(objMonts.item(i).value)){
						alert("El campo Monto a Pagar no tiene formato de número correcto.");
						ShowCommands();
						return false;
					}

					if (!ValNumDecimales(objMonts.item(i).value,2)){
						objMonts.item(i).value = "";
						ShowCommands();
						return false;
					}
					dblMontoTotal += new Number(objMonts.item(i).value);
				}
			}
			if (dblMontoTotal <= 0)
			{
				alert("Se debe ingresar un monto mayor a 0.0 para realizar el pago.");
				ShowCommands();
				return false;
			}
			return true;
	...
	<body onload="Javascript:history.go(1);RedondearInicio3();f_datos_POS();" onunload="Javascript:history.go(1);">
		<form id="frmPrincipal" method="post" runat="server">
				...
				<tr>
					<td>
						<table style="BORDER-RIGHT: #336699 2px solid; BORDER-TOP: #336699 2px solid; BORDER-LEFT: #336699 2px solid; BORDER-BOTTOM: #336699 2px solid"
							...
							<thead>
								<tr>
									<td class="TituloRConsulta" align="center">Recaudación - Resultados Búsqueda</td>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td>
										<table class="Arial12b" cellPadding="3">
											...
											<tbody>
												...
												<tr>
													...
													<td><input class="clsInputDisable" id="txtTipoCli" readOnly size="16" name="txtTipoCli" runat="server"></td>
													<td></td>
													<td></td>
												</tr>
											</tbody>
											...
											<tbody>
												<tr class="RowEven">
													<td style="HEIGHT: 20px">
														<!--PROY-27440 INI--><asp:dropdownlist id="cboTipDocumento1" tabIndex="6" runat="server" onChange="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto1', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento1');RedondearEfectivo('txtMonto1','cboTipDocumento1');f_bloqueo_fila(this,1);EstablecerMonto_Pagar('txtValorDeuda','txtMonto1', 'cboTipDocumento1');"
															CssClass="clsSelectEnable" AutoPostBack="True"></asp:dropdownlist><!--INICIATIVA-318-->
														<!--PROY-27440 FIN--></td>
													<td style="HEIGHT: 20px"><asp:textbox id="txtDoc1" tabIndex="7" runat="server" CssClass="clsInputEnable" Width="147px"></asp:textbox></td>
													<td style="HEIGHT: 20px"><asp:textbox id="txtMonto1" onkeyup="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto1', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento1');RedondearEfectivo('txtMonto1','cboTipDocumento1');"
												...
												<tr class="RowOdd">
													<td>
														<!--PROY-27440 INI--><asp:dropdownlist id="cboTipDocumento2" tabIndex="9" runat="server" onChange="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto2', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento2');RedondearEfectivo('txtMonto2','cboTipDocumento2');f_bloqueo_fila(this,2);"
															CssClass="clsSelectEnable" AutoPostBack="True"></asp:dropdownlist><!--INICIATIVA-318-->
														<!--PROY-27440 FIN--></td>
													<td><asp:textbox id="txtDoc2" tabIndex="10" runat="server" CssClass="clsInputEnable" Width="147px"></asp:textbox></td>
													<td><asp:textbox id="txtMonto2" onkeyup="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto2', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento2');RedondearEfectivo('txtMonto2','cboTipDocumento2');"
															tabIndex="8" runat="server" CssClass="clsInputEnable" Width="147px"></asp:textbox>
														<!--PROY-27440 INI--><A id="LnkPos2" style="VISIBILITY: hidden" onclick="blnClosingMod = false;f_EnvioPOS(2,'Envio POS');"><IMG id="icoTranPos2" style="CURSOR: hand" alt="Envio POS" src="../images/send-icon.png"
																border="0"> </A><A id="LnkDelPos2" style="VISIBILITY: hidden" onclick="blnClosingMod = false;f_EnvioPOS(2,'Eliminar POS');">
															<IMG id="icoDelPos2" style="CURSOR: hand" alt="Eliminar POS" src="../images/delete-icon.png"
																border="0"> </A><input id="HidTipPOS2" type="hidden" name="HidTipPOS2" runat="server">
														<!--PROY-27440 FIN--></td>
												</tr>
												<tr class="RowEven">
													<td>
														<!--PROY-27440 INI--><asp:dropdownlist id="cboTipDocumento3" tabIndex="12" runat="server" onChange="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto3', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento3');RedondearEfectivo('txtMonto3','cboTipDocumento3');f_bloqueo_fila(this,3);"
															CssClass="clsSelectEnable" AutoPostBack="True"></asp:dropdownlist><!--INICIATIVA-318-->
														<!--PROY-27440 FIN--></td>
													<td><asp:textbox id="txtDoc3" tabIndex="13" runat="server" CssClass="clsInputEnable" Width="147px"></asp:textbox></td>
													<td><asp:textbox id="txtMonto3" onkeyup="javascript:MontoEfectivoRestriccion(f_CalculoDeuda_TablaMiles('dgDocumentosPago', '6', '1'), 'txtMonto3', 'txtMonto1', 'txtMonto2', 'txtMonto3', 'cboTipDocumento3');RedondearEfectivo('txtMonto3','cboTipDocumento3');"
															tabIndex="8" runat="server" CssClass="clsInputEnable" Width="147px"></asp:textbox>
														<!--PROY-27440 INI--><A id="LnkPos3" style="VISIBILITY: hidden" onclick="blnClosingMod = false;f_EnvioPOS(3,'Envio POS');"><IMG id="icoTranPos3" style="CURSOR: hand" alt="Envio POS" src="../images/send-icon.png"
																border="0"> </A><A id="LnkDelPos3" style="VISIBILITY: hidden" onclick="blnClosingMod = false;f_EnvioPOS(3,'Eliminar POS');">
															<IMG id="icoDelPos3" style="CURSOR: hand" alt="Eliminar POS" src="../images/delete-icon.png"
										...
										<div id="divDocumentos">
											<table class="Arial12b" style="WIDTH: 100%" cellSpacing="0" cellPadding="0">
												<thead>
													<tr>
														<td class="Arial12br" vAlign="bottom" colSpan="7" height="32">Documentos</td>
													</tr>
												</thead>
												<tbody>
													<tr>
														<td colSpan="7">
															<div id="divDocumentosPago">
																<asp:datagrid id="dgDocumentosPago" tabIndex="17" runat="server" CssClass="Arial12b" Width="100%"
																	AutoGenerateColumns="False" BorderWidth="1px" HeaderStyle-CssClass="ColumnHeader" ItemStyle-Height="25px"
																	BorderColor="#393939" CellPadding="0">
																	...
																	<Columns>
																		<asp:TemplateColumn HeaderText="Tipo Documento">
																			...
																			<ItemTemplate>
																				<asp:Label id="lblDOC_TipoDocumento" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "TIPO_DOCUMENTO") %>'>
																				</asp:Label>
																			</ItemTemplate>
																		</asp:TemplateColumn>
																		<asp:TemplateColumn HeaderText="Numero Documento">
																			...
																			<ItemTemplate>
																				<asp:Label ID="lblDOC_NumeroDocumento" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "NUMERO_DOCUMENTO") %>'>
																				</asp:Label>
																			</ItemTemplate>
																		</asp:TemplateColumn>
																		<asp:TemplateColumn HeaderText="Descripci&#243;n Servicio">
																			<HeaderStyle Width="32%"></HeaderStyle>
																			<ItemTemplate>
																				<asp:Label ID="lblDOC_DescripcionServicio" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "DESCRIPCION_SERVICIO") %>'>
																				</asp:Label>
																			</ItemTemplate>
																		</asp:TemplateColumn>
																		...
																		<asp:TemplateColumn HeaderText="Total">
																			...
																			<ItemTemplate>
																				<asp:Label ID="lblDOC_Deuda" runat="server" Text='<%# DataBinder.Eval(Container.DataItem, "DEUDA","{0:#,##0.00}") %>'>
																				</asp:Label>
																				<input id="hidDOC_EsDeudaOLD" type="hidden" name="hidDOC_EsDeudaOLD" runat="server" style="WIDTH: 8px" value='<%# DataBinder.Eval(Container.DataItem, "ES_DEUDA_OLD") %>'>
																			</ItemTemplate>
																		</asp:TemplateColumn>
																	</Columns>
																</asp:datagrid></div>
					...
					<td>
						...
						<div id="Botones">
							<table borderColor="#336699" cellSpacing="0" cellPadding="0" width="400" align="center"
								...
								<tr>
									<td align="center">
										<div id="divBotones" style="DISPLAY: block">
			...
			<!-- Valor que determina que es PostBack --><input id="hdnMensaje" style="WIDTH: 8px" type="hidden" name="hdnMensaje" runat="server">
			<!-- Valor que determina que es PostBack -->
			<!-- Atributos de la Página --><input id="hdnPuntoDeVenta" style="WIDTH: 8px" type="hidden" name="hdnPuntoDeVenta" runat="server">
			<input id="hdnUsuario" type="hidden" name="hdnUsuario" runat="server" style="WIDTH: 8px">
			...
			<input id="txtNumeroTrace" style="WIDTH: 8px" type="hidden" name="txtNumeroTrace" runat="server">
			<input id="hdnRutaLog" type="hidden" name="hdnRutaLog" runat="server" style="WIDTH: 8px">
			<input id="hdnDetalleLog" type="hidden" name="hdnDetalleLog" runat="server" style="WIDTH: 8px">
			<input id="txtTipoIdentificador" type="hidden" name="txtTipoIdentificador" runat="server"
				style="WIDTH: 8px"> <input id="txtIdentificador" type="hidden" name="txtIdentificador" runat="server" style="WIDTH: 8px">
			<input id="hdnEjecutado" type="hidden" value="I" name="hdnEjecutado" runat="server" style="WIDTH: 8px"><INPUT id="hidRecibos" style="WIDTH: 8px" type="hidden" name="hidRecibos" runat="server">
			...
			<input id="hidF2" type="hidden" name="hidF2" runat="server"> <input id="hidF3" type="hidden" name="hidF3" runat="server">
			<!--PROY-27440 FIN-->
		...
		<script language="javascript">
         //document.frmPrincipal.xBoton.style="DISPLAY:none;"
