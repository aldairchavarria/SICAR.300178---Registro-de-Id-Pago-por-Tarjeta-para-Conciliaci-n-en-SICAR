...
#End Region
    ...
    Private Sub load_data_param_pos()
        ...
        Dim strPedidoLog As String
        Dim strTipoPago As String = Me.HidTipoPago.Value

        strPedidoLog = Devolver_TipoPago_POS(strTipoPago) & " Identificador : [" & Funciones.CheckStr(Request.QueryString("pIdent")) & "] "
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " === INICIO load_data_param_pos === ")
        Me.HidTipoPago.Value = ClsKeyPOS.strTipPagoPR '04  Pago de Recibos
        ...
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  HidIntAutPos : " & HidIntAutPos.Value)
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  HidGrabAuto : " & HidGrabAuto.Value)
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  HidCodOpera : " & HidCodOpera.Value)
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  HidTipoOpera : " & HidCodOpera.Value)
        ...
        If Me.HidIntAutPos.Value = "1" Then
            ...
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  strIpClient : " & strIpClient)

            'VISA INICIO
            strTipoVisa = "V"
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "== INI ConsultarDatosPOS VISA ==")
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "   PKG_SISCAJ_POS.SISCAJS_CON_POS_MC")
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "      IN strIpClient : " & strIpClient)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "      IN strIpClient : " & strIpClient)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "      IN HidPtoVenta : " & HidPtoVenta.Value)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "      IN strEstadoPos : " & strEstadoPos)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "      IN strTipoVisa : " & strTipoVisa)
            ds = objSicarDB.ConsultarDatosPOS(strIpClient, Me.HidPtoVenta.Value, strEstadoPos, strTipoVisa)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  TOTAL VISA  : " & Funciones.CheckStr(ds.Tables(0).Rows.Count))
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           HidDatoPosVisa : " & HidDatoPosVisa.Value)
            Else
                bvalida = 1
                Response.Write("<script>alert('" & strMensajeVisa & "');</script>")
            End If
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "== FIN ConsultarDatosPOS VISA ==")
            'VISA FIN

            'MC INICIO
            strTipoVisa = "M"
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "== INI ConsultarDatosPOS MCD ==")
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " - " & "   PKG_SISCAJ_POS.SISCAJS_CON_POS_MC")
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           strIpClient : " & strIpClient)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           HidPtoVenta : " & HidPtoVenta.Value)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           strEstadoPos : " & strEstadoPos)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           strTipoVisa : " & strTipoVisa)

            ds = objSicarDB.ConsultarDatosPOS(strIpClient, Me.HidPtoVenta.Value, strEstadoPos, strTipoVisa)
            objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "  TOTAL  MCD : " & Funciones.CheckStr(ds.Tables(0).Rows.Count))
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & "           HidDatoPosMC : " & HidDatoPosMC.Value)
            Else
                If bvalida = 0 Then
        ...
        End If
        objFileLog.Log_WriteLog(pathFilePos, strArchivoPos, strPedidoLog & " === FIN load_data_param_pos === ")
    ...
    Private Sub LeeParametros()
        ...
        Me.Session("PAGINA_INICIAL") = "bsqDocumentos.aspx"
    ...
    Private Sub ObtenerDocumentosPorPagar( _
        ...
        Else '** si no hay error
            ...
            hidRecibos.Value = Join(arrDetalle, ",")
        End If
    End Sub
        ...
        Select Case intTipo
            Case 0
                ...
                If Len(Trim(strFecha)) > 0 Then
                    strNuevaFecha = Mid(strFecha, 7, 2) & "/" & Mid(strFecha, 5, 2) & "/" & Mid(strFecha, 1, 4) '"12/11/2012"
    ...
    Private Sub CargarRecibosRecaudados(ByVal pRecibos() As String)
        ...
        Dim cteRECIBO_ESDEUDAOLD As Short = 8
        Dim datRecibos As New DataTable    '//tabla de recibos recaudados para cargar en grilla
        '---crea tabla
        datRecibos.Columns.Add(New DataColumn("TIPO_DOCUMENTO", GetType(String)))
        datRecibos.Columns.Add(New DataColumn("NUMERO_DOCUMENTO", GetType(String)))
        datRecibos.Columns.Add(New DataColumn("DESCRIPCION_SERVICIO", GetType(String)))
        ...
        datRecibos.Columns.Add(New DataColumn("ES_DEUDA_OLD", GetType(String)))
        '---
        If Not pRecibos Is Nothing AndAlso pRecibos.Length > 0 Then
            ...
            For i = 0 To pRecibos.Length - 1
                If Len(Trim(pRecibos(i))) > 0 Then
                    ...
                    drwFila("TIPO_DOCUMENTO") = arrRecibo(cteRECIBO_TIPODOCUMENTO)
                    drwFila("NUMERO_DOCUMENTO") = arrRecibo(cteRECIBO_NUMERODOCUMENTO)
                    drwFila("DESCRIPCION_SERVICIO") = arrRecibo(cteRECIBO_DESCRIPCIONSERVICIO)
                    ...
                    sDeuda = FormateoMonto(arrRecibo(cteRECIBO_MONTODOCUMENTO)).ToString("N2")
    ...
    Private Sub PagarDocumentos( _
        ...
        Else
            ...
            If (strNumeroRec <> "" And Funciones.CheckStr(Me.HidIdCabez.Value) <> "") Then
                ...
                objFileLog.Log_WriteLog(pathFile, strArchivo, strIdentifyLog & "- Fin Actualizar cabezera " & "	strNumeroRec: " & strNumeroRec)
            End If
    ...
    Private Sub actualizar_codigo_recuadacion(ByVal strIdRecaudacion As String, ByVal strCodCabez As String)
        ...
        Try
            ...
            If strCodRpt <> "0" Then
                ...
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Error en el Servicio " & "strCodRpt : " & strCodRpt)
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Error en el Servicio " & "strMsgRpt : " & strMsgRpt)
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Ejecutando Contingencia - Inicio")

                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Ejecutando Contingencia - SP: PKG_SISCAJ_POS.SICASU_TRANSPOS")
                objTransascPos.UpdateCabTransaccPOS(objEntity, strCodRpt, strMsgRpt)
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Ejecutando Contingencia - strCodRpt:" & strCodRpt)
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Ejecutando Contingencia - strMsgRpt:" & strMsgRpt)
                objFileLog.Log_WriteLog(pathFile, strArchivoPos, strPedidoLog & "ActualizarTransaction - Ejecutando Contingencia - Fin")
