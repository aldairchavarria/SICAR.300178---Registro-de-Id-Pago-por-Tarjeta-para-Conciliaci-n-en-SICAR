<%@ Page Language="vb" AutoEventWireup="false" Codebehind="ConsultaCajaGen.aspx.vb" Inherits="SisCajas.ConsultaCajaGen" %>
...
<html>
	<head>
		...
		function f_OpenOficina(){
			...
			window.open(Direcc,"OpenOfG","directories=no,menubar=no,scrollbars=no,top=100,resizable=no,left=185,width=420,height=450");
		}
		
		function f_OpenCajero(){
			var of = document.getElementById("hidCodOficina").value;
			Direcc = "ConsultaCajaTot_Cajero.aspx?of=" + of;
			window.open(Direcc,"OpenCajG","directories=no,menubar=no,scrollbars=no,top=100,resizable=no,left=185,width=420,height=450");
		}
		
		function f_CargarDatosOficina(){
			frmConsultaCajaGen.loadDataHandler.click();			
		}
		...
		function f_buscarCajero(){
			if(event.keyCode == 13 ){
				frmConsultaCajaGen.loadCajeroHandler.click();
			}
		}
		...
		function f_ValidarControles(){
			...
			if(vrOficinas.length === 0 && !vrChecked.checked){
				...
				if (!ValidaFechaA('document.frmConsultaCajaGen.txtFechaIni',false)) return false;
				if (!ValidaFechaA('document.frmConsultaCajaGen.txtFechaFin',false)) return false;
				if (!FechaMayorSistema('document.frmConsultaCajaGen.txtFechaIni','FECHA INICIO')) return false;
				if (!FechaMayor('document.frmConsultaCajaGen.txtFechaFin','document.frmConsultaCajaGen.txtFechaIni','FECHA FIN','FECHA INICIO')) return false;
				return true;
			}
		};
		//Fin - INI-936 - JH
	...
	<body MS_POSITIONING="GridLayout">
		<form id="frmConsultaCajaGen" method="post" runat="server">
													...
													<tr>
														...
														<td colspan="4" class="Arial12b">
															<input id="txtFechaIni" class="clsInputEnable" maxLength="10" size="12" name="txtFechaIni"
																...
																href="javascript:show_calendar('frmConsultaCajaGen.txtFechaIni');"><IMG border="0" src="../../images/botones/btn_Calendario.gif"></A>
															&nbsp;&nbsp;&nbsp; a &nbsp;&nbsp;&nbsp;&nbsp;<input id="txtFechaFin" class="clsInputEnable" maxLength="10" size="12" name="txtFechaFin"
																runat="server" style="WIDTH: 95px; HEIGHT: 17px"> <A onmouseover="window.status='Date Picker';return true;" onmouseout="window.status='';return true;"
																href="javascript:show_calendar('frmConsultaCajaGen.txtFechaFin');"><IMG border="0" src="../../images/botones/btn_Calendario.gif"></A>
														</td>
													</tr>
													<tr>
