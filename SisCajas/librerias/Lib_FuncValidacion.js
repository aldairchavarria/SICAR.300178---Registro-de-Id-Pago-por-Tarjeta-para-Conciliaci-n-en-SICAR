	...
	function ValidaHoraA(control)
{
	...
	//Inicio - INI-936 - JH - Función para comparar 2 fechas y obtener su diferencia
	function DiferenciaFechaB(control1,control2)
	{
		var diferencia;
		var cadena1;
		var cadena2;
		eval("cadena1 = " + control1 + ".value");
		eval("cadena2 = " + control2 + ".value");
		var strfecha1 = cadena1.substr(3,2)+"/"+cadena1.substr(0,2)+"/"+cadena1.substr(6,4);
		var strfecha2 = cadena2.substr(3,2)+"/"+cadena2.substr(0,2)+"/"+cadena2.substr(6,4);
		var date1 = new Date(strfecha1);
		var date2 = new Date(strfecha2);
		var difhoras = date2.getTime()-date1.getTime()
		diferencia = Math.abs(difhoras/(1000*3600*24));
		//alert(diferencia);
		return diferencia;
	}
        //Fin - INI-936
...
function ValidaAlfanumericoTicket(control,campo,vacio){
	var flag=true;
	var i,j,a,cadena;
	eval("cadena = " + control + ".value");
	cadena=trim(cadena);
...
//aotane 05.08.2013
function RedondeaInmediatoSuperior(campo) 
{	    
	var intOrigen = campo.toString();		
	var vuelto ;
	var enteroTmp = intOrigen.split(".");
	var entero = enteroTmp[0];			
	var parteDecimal = enteroTmp[1];					
	...
	else
	{
		...
		if (tmp>=2)
		{
		    ...
		    var decimalRedondear = parteDecimal.substring(1,tmp);
		  
			
			if(parseInt(decimalRedondear) >= 1)
			{	
		       
				if (campo < 0)
				{   vuelto = (parseInt(entero)) + ((parseInt(primerDigitoDecimal*-1))/10);
			...
			{	
			   vuelto = (parseInt(entero)) + (parseInt(primerDigitoDecimal)/10);
			   vuelto = vuelto.toFixed(2);
